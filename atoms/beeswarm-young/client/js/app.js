...
data.map(d => {
	...
	if(match){
		d.young = match.young_bucket;
		d.population = match.population;
	}
})

const countyIds = ['53075','12099', '13035'];

let keyWrapper = d3.select('.interactive-wrapper-bees-young')
.append("div")
...
let youngVariables = [
"Counties with under 20% aged 18-34",
"Counties with 20-25% aged 18-34",
"Counties with over 25% aged 18-34"
]

let isMobile = window.matchMedia('(max-width: 700px)').matches;

const atomEl = d3.select('.interactive-wrapper-bees-young').node();
...
let legend = d3.select('.interactive-wrapper-bees-young')
.append("div")
...
youngVariables.map(v => {

	let datum = data.filter(d => d.young === v && d.swing != null);

	let dodge = new Dodge(datum, xScale, radius, padding);

	let max = dodge.find(f => f.y == d3.max(dodge, d => d.y))

	let header = d3.select('.interactive-wrapper-bees-young')
	.append("div")
	...
	let subheader = d3.select('.interactive-wrapper-bees-young')
	.append("div")
	...
	let svg = d3.select('.interactive-wrapper-bees-young')
	.append("svg")
	.attr('id', 'svg-beeswarm-' + v.replace(/[_()-\s%$,]/g, ""))
	.attr('class', 'svg-beeswarm-young')
	.attr("width", width)
	.attr('height', 0)
