...
import data from 'assets/json/housevtrump.json'
...
let keyWrapper = d3.select('.interactive-wrapper-senate')
.append("div")
...
const atomEl = d3.select('.interactive-wrapper-senate').node();
...
let legend = d3.select('.interactive-wrapper-senate')
.append("div")
...
const makeChart = (svg, dodge, max, className) => {
	...
	let dodge = new Dodge(data, xScale, radius, padding);

	let max = dodge.find(f => f.y == d3.max(dodge, d => d.y))

	let header = d3.select('.interactive-wrapper-senate')
	.append("div")
	...
	let subheader = d3.select('.interactive-wrapper-senate')
	.append("div")
	...
	let svg = d3.select('.interactive-wrapper-senate')
	.append("svg")
